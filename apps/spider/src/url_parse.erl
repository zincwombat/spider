...
%%%----------------------------------------------------------------------
-vc('$Id: url_parse.erl,v 1.1.1.1 2008/06/18 00:56:57 thobbins Exp $ ').
-id('$Id: url_parse.erl,v 1.1.1.1 2008/06/18 00:56:57 thobbins Exp $ ').
-source('$Source: /repository/development/projects/rsp/dg/src/url_parse.erl,v $ ').
-revision('$Revision: 1.1.1.1 $ ').
-date('$Date: 2008/06/18 00:56:57 $ ').
-state('$State: Exp $ ').
...
parse_https([$/,$/|Cs]) ->
	...
	{Host,Port,[$/ | Cs1]} ->
	    case parse_hpath(Cs1) of
		{HPath, []} -> 
		    {https, Host, Port, HPath, []};
		{HPath, [$?|Cs2]} ->
		    case parse_search(Cs2) of
			{Search, []} ->
			    {https,Host,Port,HPath,Search};
			_ -> error
		    end;
		_ -> error
	    end;
