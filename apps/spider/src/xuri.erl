...
scan_pathquery(C0) ->
   Z= scan_abspath(C0),
	...
	{[],Path} ->
	    Path;
	{A,B}->		%% PATCH: Tony Hobbins 4/6/2003
		B++A
    end.
...
scan_query([H|C0],Acc) when H==$-; H==$_; H==$.; H==$!; H==$~;
			    H==$*; H==$'; H==$(; H==$) -> % mark
    scan_query(C0,[H|Acc]);
scan_query([H|C0],Acc) when H==$#; H==${; H==$} -> 	%% added TOH 11/7/2003
    scan_query(C0,[H|Acc]);
scan_query([H|C0],Acc) when H==$\\; H==$$ -> 	%% added TOH 27/10/2003
    scan_query(C0,[H|Acc]);
